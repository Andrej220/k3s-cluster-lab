#!/bin/bash
set -e
HOSTNAME=${1:-"hostnameisnotset"}
SCRIPT_DIR=$(dirname "$(realpath "${BASH_SOURCE[0]}")")
source "$SCRIPT_DIR"/config.env.sh

echo "Running Ansible playbooks to deploy K3S, Rook-Ceph, Grafana"
sudo -i -u user ansible-playbook -i /mnt/files/ansible/inventory/hosts.ini "$ANSIBLE_DIR"/k3s_lab_setup_new.yaml -e "hostname=${HOSTNAME}"
